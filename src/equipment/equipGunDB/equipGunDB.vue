    ...
    <div class="charListArea">
      <table class="charListTable">
        <tr class="outterBorder lightRoundBorder charListNavBar fullBox">
          ...
          <th class="statNav normalStatNav" @click="sortAmmoType" v-bind:class="{ active: ammotypeNavbar }">탄종</th>
          <th class="statNav normalStatNav" @click="sortFirepower" v-bind:class="{ active: firepowerNavbar }">화력</th>
        ...
        <tr class="charList" v-for="equip in filtered">
          ...
          <td class="stats">{{ equip.equipammotype }}</td>
          <td class="stats">{{ equip.equipmaxfirepower }}</td>
          <td class="stats">{{ equip.equipmaxantiair }}</td>
        </tr>
      </table>
    </div>
  </div>
...
export default {
  ...
  mounted() {
    ...
    if (sessionStorage.getItem(("equipDBgunBox")) != null) {
      this.equipBox = JSON.parse(sessionStorage.getItem("equipDBgunBox"))
      this.equipType = JSON.parse(sessionStorage.getItem("equipDBgunType"))
      this.equipTier = JSON.parse(sessionStorage.getItem("equipDBgunTier"))
      this.equipRarity = JSON.parse(sessionStorage.getItem("equipDBgunRarity"))
      this.equipName = JSON.parse(sessionStorage.getItem("equipDBgunName"))
    }
  ...
  methods: {
    checkBoxToggle() {
      ...
      this.ammotypeNavbar = false
      this.firepowerNavbar = false
      this.antiairNavbar = false
      let self = this
      sessionStorage.setItem("equipDBgunBox", JSON.stringify(this.equipBox))
      sessionStorage.setItem("equipDBgunType", JSON.stringify(this.equipType))
      sessionStorage.setItem("equipDBgunTier", JSON.stringify(this.equipTier))
      sessionStorage.setItem("equipDBgunRarity", JSON.stringify(this.equipRarity))
      sessionStorage.setItem("equipDBgunName", "null")
      this.$http.post("/equipfilter", {
        equipBox: this.equipBox,
        equipType: this.equipType,
        equipTier: this.equipTier,
    ...
    nameSearch() {
      ...
      sessionStorage.setItem("equipDBgunName", JSON.stringify(this.equipName))
    },
    sortName() {
    ...
    sortAmmoType() {
      ...
      this.maxShotSpeedNavbar = false
      this.rangeNavbar = false
      this.ammotypeNavbar = true
      this.firepowerNavbar = false
      this.antiairNavbar = false
      if (this.sortOrder == "asc") {
        this.filtered.sort(function(a,b) {return (a.equipammotype > b.equipammotype) ? 1 : ((b.equipammotype > a.equipammotype) ? -1 : 0);} );
        this.sortOrder = "desc"
      }
      else {
        this.filtered.sort(function(a,b) {return (a.equipammotype < b.equipammotype) ? 1 : ((b.equipammotype < a.equipammotype) ? -1 : 0);} );
